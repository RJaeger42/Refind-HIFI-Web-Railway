...
import type { SiteScraper, ListingResult } from '../src/types.js';
...
function findScraper(site?: string): SiteScraper | undefined {
  if (!site) return undefined;
  const target = site.toLowerCase();
  return siteScrapers.find(
    (scraper) => scraper.slug.toLowerCase() === target || scraper.name.toLowerCase().includes(target),
  );
}

function printListings(listings: ListingResult[], limit?: number): void {
