"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AudioPerformancePlaywright = void 0;
exports.createAudioPerformancePlaywright = createAudioPerformancePlaywright;
const BaseScraper_1 = require("../BaseScraper");
const utils_1 = require("../utils");
/**
 * Playwright-based scraper for Audio Performance
 * Uses Starweb e-commerce platform
 *
 * Base URL: https://www.audioperformance.se (inferred)
 * Type: Starweb e-commerce platform
 * Features: html_parsing, pagination
 */
class AudioPerformancePlaywright extends BaseScraper_1.BaseScraper {
    constructor(options) {
        super('https://www.audioperformance.se', 'Audio Performance', options);
    }
    async search(query, minPrice, maxPrice) {
        if (!query?.trim()) {
            return [];
        }
        const results = [];
        let page = 1;
        const maxPages = this.options.maxPages;
        while (page <= maxPages) {
            await this.rateLimit();
            const playwright = await this.newPage();
            try {
                // Build search URL
                const params = new URLSearchParams({
                    q: query,
                    page: page.toString(),
                });
                const searchUrl = `${this.baseUrl}/search?${params.toString()}`;
                // Navigate to search page
                await playwright.goto(searchUrl, {
                    waitUntil: 'domcontentloaded',
                    timeout: this.options.timeout,
                });
                // Extract listings
                const items = await playwright
                    .locator('ul.products li.gallery-item, .product-item')
                    .all();
                if (items.length === 0) {
                    break;
                }
                for (const item of items) {
                    try {
                        const link = item.locator('a.gallery-info-link, a[href*="/product"]').first();
                        const titleTag = item.locator('.description h3, .product-name').first();
                        const priceTag = item.locator('.product-price .amount, .price').first();
                        const url = await link.getAttribute('href');
                        const title = await titleTag.textContent();
                        const priceText = await priceTag.textContent();
                        if (!title || !url)
                            continue;
                        if (!(0, utils_1.matchesSearchQuery)(title, query)) {
                            continue;
                        }
                        const desc = await item.locator('.product-sku, .sku').textContent();
                        const imgUrl = await item.locator('img').getAttribute('data-src');
                        const loc = await item.locator('.stock-status').textContent();
                        const listing = {
                            title: title.trim(),
                            url: (0, utils_1.normalizeUrl)(url, this.baseUrl),
                            price: (0, utils_1.extractPrice)(priceText || ''),
                            description: desc || undefined,
                            imageUrl: imgUrl || undefined,
                            location: loc || undefined,
                            postedDate: undefined,
                            rawData: { source: 'starweb' },
                        };
                        results.push(listing);
                    }
                    catch (error) {
                        continue;
                    }
                }
                page++;
            }
            finally {
                await playwright.close();
            }
        }
        return (0, utils_1.filterByPrice)(results, minPrice, maxPrice);
    }
}
exports.AudioPerformancePlaywright = AudioPerformancePlaywright;
function createAudioPerformancePlaywright(options) {
    return new AudioPerformancePlaywright(options);
}
//# sourceMappingURL=AudioPerformance.js.map