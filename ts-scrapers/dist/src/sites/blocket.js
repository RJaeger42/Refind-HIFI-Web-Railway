...
export class BlocketScraper extends BaseSiteScraper {
    ...
    buildSearchUrl(query, params, pageIndex, category) {
    ...
    async collectListings(page) {
        const listings = await page.evaluate(() => {
            ...
            for (const card of cards) {
                ...
                const postedAt = (function parseTime(raw) {
                    if (!raw)
                        return null;
                ...
                const timestamp = [topInfoText, timeText].filter((segment) => segment && segment.length).join(' ') || timeText || topInfoText;
