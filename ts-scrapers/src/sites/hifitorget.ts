...
export class HifiTorgetScraper extends BaseSiteScraper {
  ...
  private buildSearchUrl(query: string, pageIndex: number): string {
    const encoded = encodeURIComponent(query);
    const base = `${this.baseUrl}/?q=${encoded}`;
    return pageIndex > 1 ? `${base}&page=${pageIndex}` : base;
  }

  private buildFallbackUrl(query: string, pageIndex: number): string {
    const encoded = encodeURIComponent(query);
    const base = `${this.baseUrl}?q=${encoded}`;
    return pageIndex > 1 ? `${base}&page=${pageIndex}` : base;
  }

  private async loadPage(page: Page, url: string) {
  ...
  private parseCard(root: ReturnType<typeof load>): TorgetListing | null {
    const anchor =
      root('a[aria-label]')
    ...
    const description =
      root('p.card-text').last().text()?.replace(/\s+/g, ' ').trim() ||
      root('p.card-text')
        .eq(1)
        .text()
        ?.replace(/\s+/g, ' ')
  ...
  private parseSwedishDate(raw: string | null): string | null {
    if (!raw) {
      return null;
    }
    ...
    const dateMatch = normalized.match(
      /^(\d{1,2})\s+(jan|feb|mar|apr|maj|jun|jul|aug|sep|okt|nov|dec)[a-zåäö]*\.?(?:\s+(\d{4}))?$/i,
    );
    if (dateMatch) {
