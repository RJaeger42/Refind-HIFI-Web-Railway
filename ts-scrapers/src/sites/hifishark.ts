...
export class HiFiSharkScraper extends BaseSiteScraper {
  ...
  async search(page: Page, params: SearchParams): Promise<ListingResult[]> {
    ...
    const { searchResults, searchInfo } = await page.evaluate(() => {
      const globalWindow = window as typeof window & {
        searchResults?: SharkResult;
        searchInfo?: SharkInfo;
      };
      return {
  ...
  private async fetchAdditionalHits(
    page: Page,
    searchInfo: SharkInfo,
    startFrom: number,
    total: number,
    ...
    while (offset < total) {
      const response = await page.evaluate(
        async ({ info, offsetValue, totalHits }) => {
  ...
  private parseHit(hit: Record<string, unknown>): ListingResult | null {
    try {
      const description = (hit.description as string) || 'No description';
      const title = description.trim();

      const priceData = (hit.price as { value?: number }) || {};
      const price = priceData.value ?? null;
      const displayPrice = (hit.display_price as string) || null;

      const relativeUrl = (hit.url as string) || '';
      if (!relativeUrl) {
        return null;
      }
      const url = this.normalizeUrl(relativeUrl);

      const imageUrl = (hit.image_url as string) || undefined;

      const location = (hit.location as { country_iso?: string }) || {};
